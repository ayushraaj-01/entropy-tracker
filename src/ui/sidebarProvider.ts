...
import { EntropyDatabase, FileEntropyData, WeeklyScore } from '../database';
 ...
 */
export class EntropySidebarProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'entropyTracker.sidebar';
  ...
  private sendData(): void {
    ...
    const filesWithHistory = topFiles.map((file) => {
      ...
      return {
        ...file,
        severity: getSeverity(file.score),
        badge: getSeverityBadge(getSeverity(file.score)),
   ...
   */
  private sendDetailData(filePath: string): void {
    if (!this._view) { return; }
    ...
    this._view.webview.postMessage({
      ...
      file: {
        ...fileData,
        severity: getSeverity(fileData.score),
        badge: getSeverityBadge(getSeverity(fileData.score)),
        history: history.map((h) => h.score),
      },
    });
  }
